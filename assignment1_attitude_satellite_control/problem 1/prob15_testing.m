...
%% USER INPUTS
h = 0.1;                     % sample time (s)
N  = 2600;                    % number of samples. Should be adjusted
...
table = zeros(N+1,14+3);        % memory allocation
...
for i = 1:N+1,
   ...
   table(i,:) = [t q' phi theta psi w' tau' THETA_d'];  % store data in table
...
tau     = table(:,12:14);
THETA_desired   = rad2deg*table(:, 15: 17); 
...
plot(t, THETA_desired(:,2), 'r--');
plot(t, THETA_desired(:,3), 'g--');
plot(t, phi, 'b');
plot(t, theta, 'r');
plot(t, psi, 'g');
hold off;
...
title('THETA desired vs actual THETA');
xlabel('time [s]'); 
ylabel('deg');